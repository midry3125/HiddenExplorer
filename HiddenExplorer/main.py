...
class RunFunction:
    def __init__(self, func, *args, _at_first=lambda: "", _at_exit=lambda: "", **kwargs):
        self.func = func
        self.args = args
        self.atfirst = _at_first
...
class MainFrame(wx.Frame):
    ...
    def set_layout(self, path, zip=None):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                bmp.Bind(wx.EVT_RIGHT_UP, RunFunction(self.show_menu, path, isdir, _at_first=RunFunction(self.paint_selected_color, panel)))
                bmp.Bind(wx.EVT_ENTER_WINDOW, RunFunction(self.paint_on_monse_color, panel, "#CCFFFF", _at_first=RunFunction(self._mouse_on_icon, True)))
                bmp.Bind(wx.EVT_LEAVE_WINDOW, RunFunction(self._mouse_on_icon, False))
        ...
        panel.Bind(wx.EVT_RIGHT_UP, RunFunction(self.show_menu, path, isdir, _at_first=RunFunction(self.paint_selected_color, panel)))
    ...
    def _mouse_on_icon(self, state):
        self.mouse_on_icon = state

    def paint_on_monse_color(self, widget, color):
        if not self.mouse_on_icon and widget != self.selected_widget:
